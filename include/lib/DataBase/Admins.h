...
class Admins : public IAdmins, public ITable {
 public:
  ...
  string getAdminsForContest(un int contestID) { return "[{}]"; }
  string getAdmin(un int adminID) { return "[{}]"; }
  string getPermissions(un int adminD) { return "[{}]"; }
  bool setPermissions(un int adminID, string permissions) { return true; }
  bool addAdmin(un int contestID, un int userID) { return true; }
